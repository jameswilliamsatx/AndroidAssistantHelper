...
/**
 * AuthenticationClient provides helper methods to initialize an manage the SpotifyActivity authorization flow.
 *
 * <p>
 * This client provides two versions of authorization:
 * <ol>
 * <li><h3>Single Sign-On using SpotifyActivity Android application with a fallback to <a href="https://accounts.spotify.com">SpotifyActivity Accounts Service</a> in a WebView</h3>
 *
 * <p>SDK will try to fetch the authorization code/access token using the SpotifyActivity Android client.
 * If SpotifyActivity is not installed on the device, SDK will fallback to the WebView based authorization
 * and open <a href="https://accounts.spotify.com">SpotifyActivity Accounts Service</a> in a dialog.
 * After authorization flow is completed, result is returned to the activity
 * that invoked the {@code AuthenticationClient}.</p>
 *
 * <p>If SpotifyActivity is installed on the device, SDK will connect to the SpotifyActivity client and
 * try to fetch the authorization code/access token for current user.
 * Since the user is already logged into SpotifyActivity they don't need to fill their username and password.
 ...
 *
 * <p>If SpotifyActivity is not installed on the device, SDK will open a dialog and load SpotifyActivity Accounts Service
 * into a WebView. User will have to enter their username and password to login to SpotifyActivity.
 ...
 * </li>
 * <li>
 * <h3>Opening <a href="https://accounts.spotify.com">SpotifyActivity Accounts Service</a> in a web browser</h3>
 * <p>
...
public class AuthenticationClient {
    ...
    /**
     * Triggers an intent to open the SpotifyActivity accounts service in a browser. Make sure that the
     ...
     * to make calls to the Web API and/or to play music with SpotifyActivity.
     *
     * @param contextActivity A context activity for the LoginActivity.
    ...
    /**
     * Opens SpotifyActivity in the Play Store or browser.
     *
     ...
     *
     * @param contextActivity The activity that should start the intent to open the download page.
     * @param campaign A SpotifyActivity-provided campaign ID. <code>null</code> if not provided.
     */
